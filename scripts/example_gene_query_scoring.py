...
async def refined_gene_query_similarity_with_existing_components(
    ...
    async with PMCClient() as pmc_client:
        ...
        # scorer = BioBERTScorer()  # For semantic similarity
        scorer = JaccardScorer()   # For term overlap (simpler, faster)
        ...
        # Step 3: Convert papers to documents format for scoring
        documents = []
        for paper in all_papers:
            doc = {
                ...
                'source_gene': getattr(paper, 'source_gene', 'unknown')
            }
            documents.append(doc)
        
        # Step 4: Score all documents against the combined query
        print(f"Computing similarities for {len(documents)} abstracts...")
        scores = scorer.score_documents(combined_query, documents)
        ...
        print(f"Average per abstract: {total_score/len(documents):.3f}")
