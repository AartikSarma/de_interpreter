...
def main():
    ...
    **Supported Omics Types**: Transcriptomics, Proteomics, Metabolomics, 
    ...
    with st.sidebar:
        ...
        max_features = st.slider(f"Max {feature_name} to analyze", 5, 50, 20,
                                help=f"Maximum number of {feature_name} for detailed literature analysis and synthesis")
        ...
        st.subheader("🏷️ MeSH Enhancement")
        use_mesh = st.checkbox("Enable MeSH term enhancement", value=False,
                              help="Use Claude Haiku to generate MeSH terms for improved literature searches")
        
        mesh_terms_count = 3  # Default
        if use_mesh:
            if not anthropic_key:
                st.warning("⚠️ MeSH enhancement requires Anthropic API key")
            else:
                st.success("✅ MeSH enhancement available")
                mesh_terms_count = st.slider("Number of MeSH terms", 1, 5, 3,
                                            help="Number of MeSH terms to generate per query")
                st.info("🔬 MeSH terms will be generated by Claude Haiku to enhance PubMed searches")
...
def run_simplified_analysis(data_file, metadata, omics_type, max_features, use_cache, use_scoring, scorer_type, biobert_model, use_mesh, mesh_terms_count, feature_name):
    ...
    try:
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config = AnalysisConfig(
                ...
                use_mesh_enhancement=use_mesh,
                mesh_terms_count=mesh_terms_count
            )
